...
class MDict(object):
    ...
    def _decode_key_block(self, key_block_compressed, key_block_info_list):
        ...
        for compressed_size, decompressed_size in key_block_info_list:
            ...
            adler32 = unpack(
                '>I', key_block_compressed[start + 4:start + 8])[0]
    ...
    def _split_key_block(self, key_block):
        ...
        while key_start_index < len(key_block):
            temp = key_block[key_start_index:key_start_index +
                             self._number_width]
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            ...
            if record_block_type == b'\x00\x00\x00\x00':
                ...
                record_block = lzo.decompress(
                    record_block_compressed[start + 8:end], initSize=decompressed_size, blockSize=1308672)
    ...
    def get_index(self, check_block=True):
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            ...
            while i < len(self._key_list):
                ...
                i += 1
                if check_block:
                    data = record_block[record_start -
                                        offset:record_end - offset]
    ...
    def _decode_record_block(self):
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            ...
            # lzo compression
                ...
                header = b'\xf0' + pack('>I', decompressed_size)
                record_block = lzo.decompress(
                    record_block_compressed[8:], initSize=decompressed_size, blockSize=1308672)
            # zlib compression
            elif record_block_type == b'\x02\x00\x00\x00':
                # decompress
                record_block = zlib.decompress(record_block_compressed[8:])
            ...
            while i < len(self._key_list):
                ...
                # offset
                record = record_block[record_start -
                                      offset:record_end - offset]
                # convert to utf-8
